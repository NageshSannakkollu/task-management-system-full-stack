...
const UpdatePage = () => {
    ...
    useEffect(() => {
        const getData = async() => {
            axios.get(`https://backend-task-management-system.netlify.app/api/tasks/${id}`)
            .then((response) => {
    ...
    const updateTaskHandler = event=> {
        ...
        axios.put(`https://backend-task-management-system.netlify.app/api/tasks/${id}`,values[0])
        .then(response => {
            navigate("/")
